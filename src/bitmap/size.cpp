...
#define FitOneSide(left, right)          \
  ...
  float scl = float(scale) / 100;        \
                                         \
  size.Scale(scl, scl);                  \
  scl = float(size.left) / float(imgsz.left);
...
SizeFunc(FitWidth) {
  FitOneSide(x, y)
      ...
      bool isRescale =
          size.GetWidth() < img.GetWidth() || flags & BITMAP_ALLOWENLARGE;
...
SizeFunc(Prepare) {
  SizeFlagFilter(BITMAP_ORIGINAL, Original) else SizeFlagFilter(
      BITMAP_FITALL,
      FitAll) else SizeFlagFilter(BITMAP_FITWIDTH,
                                  FitWidth) else SizeFlagFilter(BITMAP_FITHEIGHT,
                                                                FitHeight) return 0;
}
