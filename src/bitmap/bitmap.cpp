...
BitmapPageChangeStatus Bitmap::ChangePage(int step) {
  ...
  pos = (step > 0)                         ? pos + Get().size()
        : (pos < Get().size() && pos != 0) ? 0
                                           : pos - Get().size();
...
void Bitmap::MarkLoaded(size_t idx) {
  if (Vector::IsExist(m_item, idx)) m_item.at(idx).SetLoaded();
}
  ...
  for (const auto& it : Get()) {
    if (it->IsOk()) {
      ...
      if (!(m_flagSize & BITMAP_FITWIDTH))
        size.SetWidth(size.GetWidth() > it->GetWidth() ? size.GetWidth()
                                                       : it->GetWidth());
    }
  }
  if (size.GetHeight() < min.GetHeight()) size.SetHeight(min.GetHeight());
  if (size.GetWidth() < min.GetWidth()) size.SetWidth(min.GetWidth());
  return size;
}
...
wxVector<SBitmap*> Bitmap::Get(const wxPoint& area, const wxSize& size) {
  ...
  for (auto& it : Get())
    if (it->IsShown(area, size)) ret.push_back(it);
