...
impl<'a, T: BufRead + Seek> FramesCollector<'a, T> {
    ...
    pub async fn collect_vector(
        mut self,
    ) -> Option<Result<crate::image::ImageData, image::ImageError>> {
        loop {
    ...
    pub async fn collect_as_option_vector(self) -> Option<crate::image::ImageData> {
