...
bool ScrolledWindow::Create(wxWindow *parent, wxWindowID id, const wxPoint &pos,
...
void ScrolledWindow::AdjustScrollBar() { AdjustScrollbars(); }
...
void ScrolledWindow::DoSetVirtualSize(int width, int height) {
  ...
  if (!m_timer.IsRunning()) m_timer.StartOnce(500);
}

void ScrolledWindow::Scroll(const wxPoint &pos) { DoScroll(pos.x, pos.y); }

void ScrolledWindow::Scroll(int x, int y) { Scroll(wxPoint(x, y)); }
...
void ScrolledWindow::LineUp(wxOrientation orient, int step) {
  DoScrollLine(wxEVT_SCROLLWIN_LINEUP, orient, -step);
}

void ScrolledWindow::LineDown(wxOrientation orient, int step) {
  DoScrollLine(wxEVT_SCROLLWIN_LINEDOWN, orient, step);
}
...
void ScrolledWindow::OnScrollLine(wxScrollWinEvent &event) {
  ...
  is_bottom = is_bottom ||
              GetScrollRange(orient) < dimension::Get(GetClientSize(), orient);
  ...
  if (!is_edge || direction == wxALL)
    ...
    if (m_isFromRight && (direction == wxLEFT || direction == wxRIGHT))
      direction = (direction == wxLEFT) ? wxRIGHT : wxLEFT;
...
void ScrolledWindow::OnMouseWheel(wxMouseEvent &event) {
  ...
  orient = (event.ShiftDown()) ? wxHORIZONTAL : orient;
...
void ScrolledWindow::OnMouseMotion(wxMouseEvent &event) {
  ...
  // is pressed
  if (!event.HasAnyModifiers()) {
    if (event.Dragging() && event.LeftIsDown() && !event.RightIsDown() &&
        !event.MiddleIsDown()) {
