...
void LoadThread::SetParameter(std::shared_ptr<AbstractHandler> handler,
...
void LoadThread::SetBitmap(std::shared_ptr<Bitmap> bitmap) { bitmap_ = bitmap; }

void LoadThread::SetStart(size_t start) { load_start_ = start; }
...
wxThreadError LoadThread::Run() {
  ...
  if (GetBitmap() == NULL || GetHandler() == NULL) return wxTHREAD_MISC_ERROR;
...
void LoadThread::LoadImage(std::shared_ptr<Bitmap> bmp,
                           std::shared_ptr<AbstractHandler> handler, size_t idx,
                           wxCriticalSection& lock) {
  if (!bmp && !handler) return;

  if (!Vector::IsExist(bmp->GetAll(), idx)) return;

  if (bmp->GetAll().at(idx).IsLoaded()) return;

  wxImage img;
  if (!image_util::Load(img, *handler, idx)) return bmp->MarkLoaded(idx);
...
wxThread::ExitCode LoadThread::Entry() {
  ...
  while (!TestDestroy() &&
         (GetHandler()->IsExist(next) || GetHandler()->IsExist(prev))) {
    LoadImage(next, 1);
    LoadImage(prev, -1);
    if (TestDestroy()) break;
  }

  if (!TestDestroy()) Completed();
