...
Controller::Controller(wxWindow *parent) { parent_ = parent; }
...
ControllerExit Controller::Open(const std::string &path) {
  if (path.empty()) return kControllerPathEmpty;

  if (!bitmap_) return kControllerBitmapEmpty;

  auto opened_handler =
      std::shared_ptr<AbstractHandler>(HandlerFactory::NewHandler(path));

  if (!opened_handler) return kControllerHandlerNotFound;

  opened_handler->Traverse(true);
  if (opened_handler->GetParent()) opened_handler->GetParent()->Traverse();

  if (opened_handler->Size() == 0) return kControllerFolderEmpty;
  ...
  if (load_thread_) load_thread_->Delete();
...
void Controller::Zoom(const wxPoint &area, const wxPoint &pos,
                      const ControllerZoom &flag) {}

void Controller::SetBitmap(std::shared_ptr<Bitmap> bitmap) { bitmap_ = bitmap; }
