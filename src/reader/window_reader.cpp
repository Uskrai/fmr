...
Window::Window(wxWindow *parent, wxWindowID id, const wxPoint &pos,
  ...
  config_ = Config::Get();
  CalcScrollStep(static_cast<ScrollingType>(
      ConfRead("ScrollType", int(SCROLL_BY_WINDOW))));
  BindEvent();
};
...
void Window::BindEvent() {
  ...
  Bind(EVT_COMMAND_THREAD_UPDATE, &Window::OnThreadUpdate, this, LoadThreadID,
       ZoomThreadID);
  Bind(EVT_COMMAND_THREAD_COMPLETED, &Window::OnZoomThreadCompleted, this,
       ZoomThreadID);
}
...
std::shared_ptr<AbstractHandler> Window::NewHandler(const std::string &path) {
  ...
  if (handler) {
    ...
    if (handler->GetParent()) handler->GetParent()->Traverse();
  }
...
void Window::ReloadConfig() { config_->Flush(); }
...
void Window::OnMouseMotion(wxMouseEvent &event) {
  ...
  //                 m_zoomThread->Delete();
  //             }
  //             // m_zoomThread = new ZoomThread( this, wxTHREAD_DETACHED,
  //             ZoomThreadID );
