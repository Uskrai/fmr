...
std::string GetExtension(AbstractHandler *) { return "/"; }

std::string GetExtension(WxArchiveHandler *) { return ".zip"; }
...
void TestSizeHandler(AbstractHandler *handler, const std::string &path) {
  ...
  EXPECT_EQ(i, handler->Size())
      << "GetFirst's Output and Traverse's Output doesn't have the same size";
}
...
HANDLER_TEST(SizeTest) { TestSizeHandler(handler, path); }
...
HANDLER_TEST(StreamTest) { TEST_STREAM(handler, path); }
...
HANDLER_TEST(TraverseTest) {
  ...
  ASSERT_EQ(size, handler->Size())
      << "handler should be cleared before traversing";
}
